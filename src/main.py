...
def start_ai_threads(
    ...
    inference_method = getattr(config, 'inference_method', 'onnx')
    model = None
    rf_adapter = None

    if inference_method == 'roboflow':
        # ----------------------------------------------------------
        #  Roboflow hosted inference — no local model file needed.
        #  Frames are sent to the Roboflow cloud API and results
        #  come back as JSON.  Slower (~200-800 ms per frame) but
        #  requires zero GPU setup.
        # ----------------------------------------------------------
        try:
            from core.roboflow_utils import RoboflowInferenceAdapter

            rf_adapter = RoboflowInferenceAdapter(
                api_key=config.roboflow_api_key,
                workspace=getattr(config, 'roboflow_workspace', 'fortnite-ai-aim'),
                project=getattr(config, 'roboflow_project', 'cod-mw-warzone-catlb'),
                version=getattr(config, 'roboflow_model_version', 1),
                confidence=getattr(config, 'roboflow_confidence', 40),
            )
            rf_adapter.initialize()
            # Auto-populate model_class_names from the preset so
            # the aiming code knows which class is "head" vs "enemy".
            if not config.model_class_names and rf_adapter.class_names:
                config.model_class_names = list(rf_adapter.class_names)
                logger.info("Auto-set model_class_names from Roboflow preset: %s",
                            config.model_class_names)
            config.current_provider = 'RoboflowHostedAPI'
            logger.info("Roboflow hosted inference ready (%s/%s v%s)",
                        config.roboflow_workspace, config.roboflow_project,
                        config.roboflow_model_version)
        except Exception as e:
            logger.error("Roboflow adapter init failed: %s", e)
            logger.error("Falling back to local ONNX inference.")
            inference_method = 'onnx'
            rf_adapter = None

    if inference_method != 'roboflow':
        # ----------------------------------------------------------
        #  Local ONNX inference (default, fast).
        # ----------------------------------------------------------
        if not model_path.endswith('.onnx'):
            logger.error("Only .onnx model files are supported: %s", model_path)
            return False
        ...
        try:
            ...
            if actual_providers:
                ...
                logger.info("ONNX model loaded with provider: %s", actual_providers[0])
            else:
                logger.warning("Could not determine ONNX provider")
                config.current_provider = 'DmlExecutionProvider'
        except Exception as e:
            logger.error("Failed to load ONNX model: %s", e)
            logger.error("Make sure onnxruntime-directml is installed and DirectML is supported")
            return False

    ai_thread = threading.Thread(
        target=ai_logic_loop,
        args=(config, model, 'onnx', overlay_boxes_queue, overlay_confidences_queue, auto_fire_boxes_queue),
        kwargs={'roboflow_adapter': rf_adapter},
        daemon=True
    )
