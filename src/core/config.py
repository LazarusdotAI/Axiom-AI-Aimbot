...
class Config:
    ...
    def __init__(self) -> None:
        ...
        # Model architecture type — controls preprocessing and output parsing.
        # "yolov8" (default): Direct resize to model_input_size, output (1, 4+C, N).
        # "yolov5": Letterbox resize (preserves aspect ratio, gray padding),
        #           output (1, N, 5+C) with xyxy coords + obj_conf + class confs.
        # "auto":   Auto-detect the output format at runtime (safe default).
        #           Preprocessing still uses direct resize in auto mode.
        self.model_type: str = "auto"

        # Multi-class model support
        # When empty, the model is treated as single-class (legacy behavior).
        # For the Roboflow COD MW Warzone dataset, set to ["enemy", "head"].
        # The index in this list must match the class index the model outputs.
        self.model_class_names: List[str] = []

        # ── Roboflow hosted inference settings ──
        # inference_method: "onnx" (default, fast local) or "roboflow" (hosted API).
        # When set to "roboflow", frames are sent to the Roboflow API for
        # inference instead of running a local ONNX model.  This is slower
        # but requires no local GPU or ONNX model file.
        self.inference_method: str = "onnx"
        # Your Roboflow API key (found at https://app.roboflow.com → Settings → API Key).
        self.roboflow_api_key: str = ""
        # Roboflow workspace / project / version for the COD MW Warzone model.
        self.roboflow_workspace: str = "fortnite-ai-aim"
        self.roboflow_project: str = "cod-mw-warzone-catlb"
        self.roboflow_model_version: int = 1
        # Confidence threshold used by the Roboflow hosted model (0-100).
        self.roboflow_confidence: int = 40
        ...
        # 滑鼠控制方式
        # Default to 'xbox' because this build is optimised for virtual gamepad
        # output via ViGEmBus.  Switch to 'mouse_event' only if vgamepad is
        # unavailable on the target machine.
        self.mouse_move_method: str = "xbox"
        ...
        # When True, holding Right Trigger (RT) on a physical XInput controller
        # activates aim assist the same way any AimKey does.
        self.controller_aim_enabled: bool = False

        # Sticky Aim (Magnetic Aim Assist)
        # When enabled, the reticle is gently pulled toward the nearest
        # detected target.  The pull is subtle — it feels like magnetism,
        # not a hard snap.  Strength controls how aggressive the pull is:
        #   0.0 = no pull (disabled)
        #   0.3 = gentle tug  (recommended starting point)
        #   1.0 = maximum pull
        # The pull automatically tapers off when the target is far from
        # the crosshair (distance-based falloff).
        self.sticky_aim_enabled: bool = False
        self.sticky_aim_strength: float = 0.3
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'roboflow_workspace': self.roboflow_workspace,
            'roboflow_project': self.roboflow_project,
            'roboflow_model_version': self.roboflow_model_version,
            'roboflow_confidence': self.roboflow_confidence,
            ...
            'controller_aim_enabled': self.controller_aim_enabled,
            'sticky_aim_enabled': self.sticky_aim_enabled,
            'sticky_aim_strength': self.sticky_aim_strength,
