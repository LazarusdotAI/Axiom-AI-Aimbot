...
class AimPage(BasePage):
    ...
    def __init__(self, parent=None):
        ...
        # Periodically refresh the Xbox connection badge so it stays in sync
        # with the virtual controller state without requiring a manual click.
        self._xboxPollTimer = QTimer(self)
        self._xboxPollTimer.setInterval(2000)  # 2 s
        self._xboxPollTimer.timeout.connect(self._updateXboxConnectionStatus)
        self._xboxPollTimer.start()
    ...
    def _loadFromConfig(self):
        ...
        # If the saved method is already 'xbox', auto-connect the virtual
        # controller on startup so the status badge reflects reality immediately.
        if getattr(self._config, 'mouse_move_method', '') == 'xbox':
            try:
                from win_utils import is_xbox_connected, connect_xbox
    ...
    def _onMouseMoveChanged(self, text):
        if self._config:
            ...
            if text == "ddxoft":
                ...
                # Auto-connect the virtual controller the moment the user
                # selects the Xbox method so the status badge turns green
                # immediately instead of waiting for a manual button click.
                try:
                    from win_utils import is_xbox_connected, connect_xbox
                    if not is_xbox_connected():
                        connect_xbox()
