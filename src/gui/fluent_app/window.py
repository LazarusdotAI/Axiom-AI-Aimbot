...
from PyQt6.QtCore import QUrl, QSize, QTimer, QObject, Qt
from PyQt6.QtGui import QIcon, QDesktopServices, QColor, QKeyEvent
...
# ---------------------------------------------------------------------------
# Controller UI navigation dispatcher
# ---------------------------------------------------------------------------

class ControllerNavDispatcher(QObject):
    """Polls the physical Xbox controller on a QTimer and converts D-pad /
    A / B button presses into Qt key events delivered to the focused widget.

    D-pad Up/Down/Left/Right → Qt.Key_Up / Down / Left / Right
    A button                 → Qt.Key_Return  (select / activate)
    B button                 → Qt.Key_Escape  (cancel / back)

    Auto-repeat behaviour mirrors a keyboard:
      - First event fires immediately on the rising edge.
      - After REPEAT_DELAY seconds of continuous hold, events repeat every
        REPEAT_INTERVAL seconds.

    The dispatcher is started via start() and stopped via stop().  It is a
    no-op when the controller is not connected, so there is no polling
    overhead when the physical controller is absent.
    """

    # How often to poll XInput (ms).  50 ms = 20 Hz — fast enough for UI.
    POLL_MS = 50

    # Auto-repeat settings (seconds)
    REPEAT_DELAY    = 0.40   # pause before repeating starts
    REPEAT_INTERVAL = 0.10   # interval between repeated events

    # Left-stick tilt must exceed this (0-1) to count as a navigation input.
    # 0.45 = ~halfway, comfortable to trigger intentionally, avoids drift.
    STICK_THRESHOLD = 0.45

    # Map each (dpad_flag, held_attr, pressed_attr) → Qt key
    _DPAD_MAP = [
        ("dpad_up",    "dpad_up_pressed",    Qt.Key.Key_Up),
        ("dpad_down",  "dpad_down_pressed",  Qt.Key.Key_Down),
        ("dpad_left",  "dpad_left_pressed",  Qt.Key.Key_Left),
        ("dpad_right", "dpad_right_pressed", Qt.Key.Key_Right),
    ]

    # Left-stick nav: (state_attr, multiplier, internal_track_key, qt_key)
    # Positive Y = up in XInput after normalization (+1 * value > threshold → up).
    # Positive X = right.
    _STICK_MAP = [
        ("left_stick_y", +1, "lstick_up",    Qt.Key.Key_Up),
        ("left_stick_y", -1, "lstick_down",  Qt.Key.Key_Down),
        ("left_stick_x", +1, "lstick_right", Qt.Key.Key_Right),
        ("left_stick_x", -1, "lstick_left",  Qt.Key.Key_Left),
    ]
    ...
    def __init__(self, app: QApplication, parent: QObject = None) -> None:
        ...
        # Import the singleton handler — lazy so no crash if xinput is absent
        from win_utils.xinput_handler import xbox_input_handler
        self._handler = xbox_input_handler

        # Auto-repeat tracking: maps held_attr_name → timestamp of first held
    ...
    def _tick(self) -> None:
        state = self._handler.poll()
        if not state.connected:
            # Clear repeat tracking when controller is disconnected
        ...
        for held_attr, pressed_attr, qt_key in self._DPAD_MAP:
            is_held    = getattr(state, held_attr)
            is_pressed = getattr(state, pressed_attr)  # rising-edge

            if is_pressed:
                # Rising edge: fire immediately and record hold-start time
                self._send_key(qt_key)
                self._held_since[held_attr]  = now
                self._last_repeat[held_attr] = now

            elif is_held:
                # Still held: check whether it is time for another repeat
                held_for = now - self._held_since.get(held_attr, now)
                if held_for >= self.REPEAT_DELAY:
                    since_last = now - self._last_repeat.get(held_attr, now)
                    if since_last >= self.REPEAT_INTERVAL:
                        self._send_key(qt_key)
                        self._last_repeat[held_attr] = now

            else:
                # Button released — clear tracking
                self._held_since.pop(held_attr, None)
                self._last_repeat.pop(held_attr, None)

        # --- Left stick navigation with auto-repeat ---
        # Each direction is treated identically to a D-pad direction:
        # tilt past STICK_THRESHOLD fires one event immediately, then repeats
        # at the standard keyboard-repeat rate while the stick stays tilted.
        for axis_attr, sign, track_key, qt_key in self._STICK_MAP:
            value = getattr(state, axis_attr)
            is_active = (value * sign) > self.STICK_THRESHOLD

            if is_active:
                if track_key not in self._held_since:
                    # First frame past threshold: fire immediately
                    self._send_key(qt_key)
                    self._held_since[track_key]  = now
                    self._last_repeat[track_key] = now
                else:
                    # Still tilted: auto-repeat after initial delay
                    held_for   = now - self._held_since[track_key]
                    since_last = now - self._last_repeat[track_key]
                    if held_for >= self.REPEAT_DELAY and since_last >= self.REPEAT_INTERVAL:
                        self._send_key(qt_key)
                        self._last_repeat[track_key] = now
            else:
                # Stick returned to centre — clear tracking for this direction
                self._held_since.pop(track_key, None)
                self._last_repeat.pop(track_key, None)

        # --- A button → Return (select) ---
        if state.a_pressed:
    ...
    def _send_key(self, key: Qt.Key) -> None:
        """Post a press+release key event pair to the currently focused widget."""
        widget = self._app.focusWidget()
        if widget is None:
            return
        ...
        self._app.postEvent(widget, press)
        self._app.postEvent(widget, release)
...
class AxiomWindow(FluentWindow):
    def __init__(self):
        ...
        # Controller UI navigation dispatcher (started once config is loaded)
        self._controller_nav: ControllerNavDispatcher | None = None
    ...
    def setConfig(self, config):
        ...
        # Start (or keep running) the dispatcher whenever config is set/refreshed.
        # The feature is always active when the controller is connected regardless
        # of the "Controller Aim Support" toggle — navigation is never harmful.
        self._start_controller_nav()

    def _start_controller_nav(self) -> None:
        """Create and start the ControllerNavDispatcher if not already running."""
        if self._controller_nav is None:
            app = QApplication.instance()
            if app is not None:
                self._controller_nav = ControllerNavDispatcher(app, parent=self)
                self._controller_nav.start()
    ...
    def closeEvent(self, event):
        """視窗關閉時自動保存配置"""
        # Stop the controller navigation dispatcher cleanly before closing
        if self._controller_nav is not None:
            self._controller_nav.stop()
