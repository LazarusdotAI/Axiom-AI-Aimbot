...
class TestPostprocessOutputs:
    ...
    def test_no_detections(self):
        ...
        boxes, confs, class_ids = postprocess_outputs(outputs, 640, 640, 640, 0.5)
        assert boxes == []
        assert confs == []
        assert class_ids == []
    ...
    def test_single_detection(self):
        ...
        assert class_ids == [0]  # single-class → always 0
    ...
    def test_scale_factor(self):
        ...
        outputs = self._make_output([[320, 320, 100, 100, 0.9]])
        boxes, _, _ = postprocess_outputs(outputs, 1280, 720, 640, 0.5)
        x1, y1, x2, y2 = boxes[0]
    ...
    def test_multi_class_detection(self):
        """Multi-class model: 2 classes (enemy=0, head=1)"""
        from core.inference import postprocess_outputs
        # 6 columns: cx, cy, w, h, enemy_conf, head_conf
        det = np.array([
            [320, 320, 100, 200, 0.1, 0.95],  # head wins (class 1)
            [160, 160, 80, 180, 0.85, 0.2],    # enemy wins (class 0)
        ], dtype=np.float32)
        arr = det.T.reshape(1, 6, -1)
        boxes, confs, class_ids = postprocess_outputs([arr], 640, 640, 640, 0.5)
        assert len(boxes) == 2
        # class_ids should reflect which class had higher confidence
        assert 1 in class_ids  # head
        assert 0 in class_ids  # enemy

    def test_yolov5_single_class(self):
        """YOLOv5 format: (1, N, 6) with [x1, y1, x2, y2, obj_conf, cls_conf]"""
        from core.inference import postprocess_outputs
        # Need N >= 20 to trigger YOLOv5 auto-detection.
        # 1 real detection + 19 zero-confidence padding rows.
        real = np.array([[100, 100, 200, 200, 0.9, 0.95]], dtype=np.float32)
        padding = np.zeros((19, 6), dtype=np.float32)
        det = np.vstack([real, padding])
        arr = det.reshape(1, 20, 6)  # (1, N=20, 6) — YOLOv5 shape
        boxes, confs, class_ids = postprocess_outputs([arr], 640, 640, 640, 0.5)
        assert len(boxes) == 1
        # Effective conf = obj_conf * cls_conf = 0.9 * 0.95 = 0.855
        assert abs(confs[0] - 0.855) < 0.01
        assert class_ids == [0]

    def test_yolov5_multi_class(self):
        """YOLOv5 format: 2 classes — [x1, y1, x2, y2, obj_conf, cls0, cls1]"""
        from core.inference import postprocess_outputs
        real = np.array([
            [50, 50, 150, 300, 0.9, 0.1, 0.95],   # class 1 wins (head)
            [200, 100, 300, 350, 0.85, 0.9, 0.1],  # class 0 wins (body)
        ], dtype=np.float32)
        padding = np.zeros((18, 7), dtype=np.float32)
        det = np.vstack([real, padding])
        arr = det.reshape(1, 20, 7)  # (1, N=20, 7)
        boxes, confs, class_ids = postprocess_outputs([arr], 640, 640, 640, 0.5)
        assert len(boxes) == 2
        assert 1 in class_ids
        assert 0 in class_ids

    def test_yolov5_low_objectness_filtered(self):
        """YOLOv5: low obj_conf should be filtered even if cls_conf is high"""
        from core.inference import postprocess_outputs
        real = np.array([[100, 100, 200, 200, 0.1, 0.99]], dtype=np.float32)
        padding = np.zeros((19, 6), dtype=np.float32)
        det = np.vstack([real, padding])
        arr = det.reshape(1, 20, 6)
        boxes, confs, class_ids = postprocess_outputs([arr], 640, 640, 640, 0.5)
        assert len(boxes) == 0

    def test_yolov5_letterbox_preprocess(self):
        """YOLOv5 letterbox should produce correct shape with padding"""
        from core.inference import preprocess_image
        # Non-square image
        img = np.random.randint(0, 255, (480, 640, 3), dtype=np.uint8)
        result = preprocess_image(img, 640, model_type="yolov5")
        assert result.shape == (1, 3, 640, 640)
        assert result.max() <= 1.0 + 1e-6

    def test_format_autodetect(self):
        """_detect_model_format should correctly distinguish YOLOv5 vs YOLOv8"""
        from core.inference import _detect_model_format
        # YOLOv8: (1, 5, 8400) — dim1=5 (attributes), dim2=8400 (detections)
        assert _detect_model_format(np.zeros((1, 5, 8400))) == "yolov8"
        # YOLOv5: (1, 100, 6) — dim1=100 (detections), dim2=6 (attributes)
        assert _detect_model_format(np.zeros((1, 100, 6))) == "yolov5"
        # Small tensors (few detections) → default to YOLOv8 to avoid misclassification
        assert _detect_model_format(np.zeros((1, 5, 2))) == "yolov8"
        assert _detect_model_format(np.zeros((1, 6, 2))) == "yolov8"
...
class TestNonMaxSuppression:
    ...
    def test_single_box(self):
        ...
        assert len(boxes) == 1
        assert cids == [0]  # default class_id when none provided
    ...
    def test_class_ids_preserved(self):
        """class_ids should survive NMS filtering"""
        from core.inference import non_max_suppression
        boxes_in = [[10, 10, 50, 50], [200, 200, 250, 250]]
        confs_in = [0.9, 0.8]
        class_ids_in = [0, 1]
        boxes, confs, cids = non_max_suppression(
            boxes_in, confs_in, class_ids=class_ids_in,
        )
        assert len(cids) == 2
        assert set(cids) == {0, 1}
